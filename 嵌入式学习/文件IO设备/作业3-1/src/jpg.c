...
void show_image(struct lcdinfo *LCD, const char *filename)
{
    ...
    double scale = fmin(1.0, fmin((double)LCD->width / jpg_w, (double)LCD->height / jpg_h));
    int dst_w = (int)(jpg_w * scale);
    int dst_h = (int)(jpg_h * scale);
    if(dst_w < 1) dst_w = 1;
    if(dst_h < 1) dst_h = 1;
    ...
    {
        int sy = dj * jpg_h / dst_h;
        for(int di = 0; di < dst_w; di++)
        {
            int sx = di * jpg_w / dst_w;

            unsigned char *src = (unsigned char *)rgbdata + sy*jpg_pitch + sx*3;
